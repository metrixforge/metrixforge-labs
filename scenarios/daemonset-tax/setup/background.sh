...
# tools + this scenario's seed script into /opt/lab, from raw github (curl) or the
# local repo when LABS_ROOT is set (kind e2e). Ends by touching /tmp/.setup-done.
...
get scenarios/daemonset-tax/setup/seed-leak.sh "$LAB/seed-leak.sh"
...
echo "== wait for agents + app to schedule =="
kubectl -n platform rollout status ds/security-agent --timeout=120s || true
